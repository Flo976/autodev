...
function safeCheckoutMain(config) {
  try {
    ...
    // Uncommitted changes block checkout — stash and retry
    try {
      log("Stashing uncommitted changes before checkout...", config);
      git(config, "stash --include-untracked");
      git(config, "checkout main");
      git(config, "stash drop");
    } catch (e) {
...
export async function runBatch(config, { autoClose = false, dryRun = false, sprint = null } = {}) {
  // Phase 1: Collect
  const tickets = await collectTickets(config, { sprint });
