...
async function findReleaseTickets(config) {
  const jql = encodeURIComponent(
    `project=${config.projectKey} AND status=${config.statuses.DONE} AND fixVersion is EMPTY AND sprint in closedSprints() ORDER BY created ASC`
  );
...
function detectNextVersion(config) {
  ...
  try {
    ...
    // Increment minor version (x.Y.z), reset patch to 0
    if (parts.length >= 3) {
      parts[1]++;
      parts[2] = 0;
    } else {
...
function createGitHubRelease(config, tag, notes) {
  log(`Creating GitHub Release: ${tag}...`, config);
  ...
  log(`GitHub Release created: ${url}`, config);
  return url;
}
...
async function publishChangelog(config, versionName, notes) {
  ...
  try {
    ...
    logError(`Changelog publish failed: ${e.message}`, config);
    return null;
  }
}
...
export async function performRelease(config, { version, auto = false, dryRun = false }) {
  ...
  log(`=== Release ${versionName} ===`, config);

  // 1. Find tickets
  const tickets = await findReleaseTickets(config);
  log(`${tickets.length} tickets to release`, config);

  if (tickets.length === 0) {
    log("No unreleased tickets found. Exiting.", config);
    return null;
  }
  ...
  if (dryRun) {
    log("DRY RUN — Release preview:", config);
    console.log(notes);
  ...
  log(`Assigned fixVersion to ${tickets.length} tickets`, config);
  ...
  if (confluenceUrl) log(`Changelog published: ${confluenceUrl}`, config);

  // 7. Mark version as released
  await releaseVersion(config, jiraVersion.id);

  log(`=== Release ${versionName} complete ===`, config);
