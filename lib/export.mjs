...
async function fetchDoneKeys(config, { sprint } = {}) {
  ...
  while (true) {
    let url = `/rest/api/3/search/jql?jql=${encoded}&maxResults=${maxResults}&fields=key`;
    if (nextPageToken) {
      url += `&nextPageToken=${encodeURIComponent(nextPageToken)}`;
    }

    const data = await jiraFetch(config, url);
    ...
    log(`Fetched ${keys.length} done ticket keys...`);
