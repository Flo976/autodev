...
import { git, isWorktree } from "./git.mjs";
...
export async function checkSprintCompletion(config, sprintName) {
  log(`[SPRINT] Checking if "${sprintName}" is complete...`, config);
  ...
  log(`[SPRINT] ${remaining} ticket(s) remaining in "${sprintName}"`, config);
  return remaining === 0;
}
...
export async function generateSprintRecap(config, sprintName) {
  ...
  log(`[SPRINT] Generating recap for Sprint ${sprintNum}...`, config);
  ...
  try {
    ...
    log(`[SPRINT] Warning: could not fetch PRs: ${e.message}`, config);
  }
...
export async function createSprintBranch(config, sprintName) {
  ...
  log(`[SPRINT] Creating branch: ${branch}`, config);
  ...
  log(`[SPRINT] Wrote ${filePath}`, config);
  ...
  log(`[SPRINT] Sprint ${sprintNum} completed! Recap created.`, config);
  return prUrl;
}
