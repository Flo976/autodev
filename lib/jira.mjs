...
export async function findNextTickets(config, maxCount = 1, skipKeys = new Set()) {
  if (maxCount === 1) {
    const key = await findNextTicket(config, skipKeys);
    return key ? [{ key, summary: "" }] : [];
  }

  log(`Searching for up to ${maxCount} independent tickets...`, config);
  const jql = encodeURIComponent(
    `project=${config.projectKey} AND status=${config.statuses.TODO} AND sprint in openSprints() AND assignee is EMPTY ORDER BY created ASC`
  );
...
export async function fetchTicketsBatch(config, keys) {
  log(`Fetching ${keys.length} tickets in parallel...`, config);
  const results = await Promise.all(
    keys.map((key) => fetchTicket(config, key))
  );
