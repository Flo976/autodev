...
export async function findNextTicket(config, skipKeys = new Set()) {
  ...
  if (!data.issues || data.issues.length === 0) {
    log("No tickets found in active sprint. Have you started the sprint in Jira?");
    return null;
  }
...
export async function getBoardId(config) {
  ...
  const data = await agileFetch(
    config,
    `/rest/agile/1.0/board?projectKeyOrId=${config.projectKey}&type=scrum`
  );
  if (!data.values || data.values.length === 0) {
    throw new Error(`No scrum board found for project ${config.projectKey}`);
  }
  const boardId = data.values[0].id;
  log(`Board detected: ${boardId} (${data.values[0].name})`);
  return boardId;
}
